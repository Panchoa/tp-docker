        ...
        if (mysqli_query($link, $sql)) {
            ...
            foreach ($_FILES["PRO_ressources"]["error"] as $key => $error) {
                if ($error == UPLOAD_ERR_OK) {
                    ...
                    $url = "uploads/$name";
                    $minio->putObject($name, $tmp_name);

                    $sql = "INSERT INTO ressources (RE_type,RE_url,RE_nom,PRO_id) VALUES ('obj','$url','$name','$PRO_id')";
        ...
        if (isset($_POST['RE_id'])) {
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if (mysqli_query($link, $sql)) {
                    ...
                    } elseif ("obj" == $ressource['RE_type']) {
                        $minio->deleteObject($ressource['RE_nom']);
                    }
                    echo 'OK';
                } else {
                    echo 'NOK';
